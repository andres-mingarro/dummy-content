import type { TextureRenderMode } from "./index";

// tile — repeating pattern 120×120
export const DEFAULT_COLORS: Record<string, string> = {
  "primary-color":   "#00bb77",
  "secondary-color": "#000000",
};

export function buildInner(colors: Record<string, string> = {}): string {
  const c = { ...DEFAULT_COLORS, ...colors };
  return `<rect fill='${c["primary-color"]}' width='120' height='120'/><polygon fill='${c["secondary-color"]}' fill-opacity='.1' points='120 0 120 60 90 30 60 0 0 0 0 0 60 60 0 120 60 120 90 90 120 60 120 0'/>`;
}

export const RENDER_MODE: TextureRenderMode = { type: "tile", width: 120, height: 120 };
